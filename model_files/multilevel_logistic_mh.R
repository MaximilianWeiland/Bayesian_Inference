log_posterior <- function(beta, sigma_beta, gamma, X, U, y, country, J, K, L) {
  ...
  log_prior_sigma_beta <- sum(dnorm(sigma_beta, mean = 0, sd = 10, log = TRUE))
  
  # prior on gamma
  log_prior_gamma <- sum(dnorm(gamma, 0, 10, log = TRUE))
...
mh <- function(X, U, y, country,
               n_samples = 1500, warmup = 500, proposal_sd = 0.01,
