...
model {
  ...
  LKJ ~ lkj_corr_cholesky(2);  // regularized prior on correlation 
  D ~ normal(0, 0.5);  // regularized prior on the scale
  matrix[K,K] Sigma;  // matrix to store covariance matrix of beta
  Sigma = diag_pre_multiply(D, LKJ);  // prior for Sigma  D*L
  
  // regularized prior on all gamma values
  to_vector(gamma) ~ double_exponential(0, 0.1);
